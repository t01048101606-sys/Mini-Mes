...
역할에 따라 필요한 화면만 보이도록 홈 메뉴와 페이지 접근을 함께 제한.
...
> 각 단계마다 스크린샷을 넣어두면, 발표 전 리허설 자료로도 쓸 수 있고
> 라이브 데모가 여의치 않을 때 백업 자료로도 활용할 수 있습니다.
...
![품목 관리 화면](./screenshots/02_item.png)
...
![BOM 등록 화면](./screenshots/03_bom.png)
...
![원자재 입고 화면](./screenshots/04_receipt.png)
...
![생산계획 부족확인 화면](./screenshots/05_plan.png)

- 생산계획 등록 후, "계획 대비 원자재 부족 확인" 탭으로 이동
- "계획한 수량을 만들기에 재고가 충분한지 미리 확인 가능. 부족하면 빨간색으로 바로 표시." ⭐ 하이라이트

### (6) 생산 등록

![생산 등록 화면](./screenshots/06_production.png)
...
![불량검사 화면](./screenshots/07_inspection.png)
...
![출하 관리 화면](./screenshots/08_shipment.png)

- FAIL로 처리했던 LOT를 출하 목록에서 선택 시도
- "불합격 판정된 제품은 시스템이 자동으로 출하를 막아버림. 사람이 실수로 놓쳐도 시스템이 걸러주는것." ⭐ 하이라이트

### (9) LOT 추적

![LOT 추적 화면](./screenshots/09_trace.png)
...
![재고 현황 대시보드](./screenshots/10_dashboard.png)

- 잔량, 유효기한 임박 LOT 확인
- "한눈에 재고 상태를 볼 수 있는 대시보드."

> **TODO**: `screenshots/` 폴더를 만들고 위 파일명대로 캡처 이미지를 넣어주세요.
> 이미지가 없는 동안에는 GitHub에서 깨진 링크로 보이니, 캡처 전까지는
> 이 TODO 문구를 지우지 말고 남겨두는 걸 추천합니다.
